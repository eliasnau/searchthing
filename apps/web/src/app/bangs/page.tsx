...
export default function BangsPage() {
	...
	const categories = useMemo(
		() => Array.from(new Set(bangs.map((b) => b.category))),
		[],
	);
			...
			<main className="container relative flex-1 px-4 py-12 mx-auto max-w-5xl">
				<div className="mb-8 space-y-6 text-center">
					...
					<p className="mx-auto max-w-2xl text-muted-foreground">
						Use <code className="bg-muted px-1 py-0.5 rounded text-xs">!</code>{" "}
						followed by a command to search directly on your favorite sites.
					</p>
				</div>
				...
				<div className="mt-4 rounded-md border px-4 py-3 bg-muted/50">
					...
					<p className="mt-2 text-xs text-muted-foreground">
						...
						<code className="bg-muted px-1 py-0.5 rounded">
							curl -s https://searchthing.xyz/api/bangs | jq
						</code>
					</p>
				</div>
									...
									<div className="flex justify-between items-start gap-4">
										<div>
											<div className="flex items-center gap-3">
												<div className="px-3 py-1 rounded-md bg-purple-600 text-white font-medium">
													{bangToken}
												</div>
				...
				<Dialog>
					<DialogTrigger
						render={<Button className="flex fixed right-6 bottom-6 z-10" />}
					>
						<Plus /> Add your own
					</DialogTrigger>
					<DialogContent className="sm:max-w-md">
						<DialogHeader>
							...
							<DialogDescription className="text-base">
								Help improve this list by submitting a pull request or using the
								web form.
							</DialogDescription>
						</DialogHeader>
						...
						<div className="py-4 space-y-6">
							<div className="p-4 space-y-3 rounded-lg border transition-colors hover:bg-accent/50">
								...
								<p className="text-sm text-muted-foreground">
									Faster processing: submit a PR with the new bang.
								</p>
								<Link
									href="https://github.com/eliasnau/searchthing"
									target="_blank"
